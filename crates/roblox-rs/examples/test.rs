...
pub fn main() {
    ...
    Workspace::instance()
        .on_instance_changed_async(|v| async move {
            println!("connected async! {v}, {}", task::wait(1.0).await);
        })
        .leak();
    ...
    let connection = Workspace::instance().on_instance_changed(|_name| {
    ...
    my_func.on_server_invoke_async(move |player, values| async move {
        println!(
